...
typedef std::tuple<int, int, int, Parse_tree*>          stack_quad;
...
public:
    ...
    Parse_tree  run_CYK(float & parse_proba){
        ...
        Parse_tree          result(_root_symbol);
        ...
        int right_index;
        int cut;
        Parse_tree*         current_parent;
        Parse_tree*         current_left;
        Parse_tree*         current_right;
        while(! stack.empty()){
            ...
            if(cut == -1){
                ...
                current_left = new Parse_tree(_index_to_non_term.at(left_symbol_index));
                current_right = new Parse_tree(_index_to_non_term.at(right_symbol_index));
