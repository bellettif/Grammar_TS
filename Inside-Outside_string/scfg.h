...
public:
    ...
    const T_vect get_index_to_term_vect() const{
        T_vect result (_index_to_term.size());
        for(auto xy : _index_to_term){
    ...
    const int_vect get_index_to_non_term_vect() const{
        int_vect result (_index_to_non_term.size());
        for(auto xy : _index_to_non_term){
            result[xy.first] = xy.second;
        }
        return result;
