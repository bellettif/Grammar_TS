...
class Parse_tree;
std::ostream& operator<<(std::ostream& out, const Parse_tree & p);


class Parse_tree{

typedef Stochastic_rule      SGrammar_T;
...
public:
    ...
    void set_non_terminal(Parse_tree * left, Parse_tree * right){
        _isterm = false;
        _left_child = left;
        _left_rule_name = _left_child->_lhs;
        _right_child = right;
    ...
    std::string to_string(){
        ...
        if(_isterm){
            ...
            result += _left_child->to_string() + " " + _right_child->to_string();
        }
        return result;
    }

    void print_all_tree(){
        std::list<Parse_tree *>  current;
        std::list<Parse_tree *>  next;
        current.push_back(this);
        std::cout << _lhs << std::endl;
        int counter;
        while(!current.empty()){
            counter = 1;
            for(Parse_tree* x : current){
    ...
    Parse_tree* get_left_child() const{
        return _left_child;
    }
    ...
    Parse_tree* get_right_child() const{
        return _right_child;
    }
