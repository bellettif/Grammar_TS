...
int main(){
    ...
    float_vect             B_non_term_w ({0.2, 0.3, 0.4, 0.1});
    pair_i_i_vect           B_non_term_s ({B_pair_1,
                                           B_pair_2,
                                           B_pair_3,
                                           B_pair_4});
    T_vect                  B_term_s({"Pierre", "Mathieu"});
    float_vect             B_term_w({1.0, 0.2});
    SRule_T                 B_rule(2,
                                   B_non_term_w,
                                   B_non_term_s,
                                   my_rng,
    ...
    float_vect             S_non_term_w ({0.6, 0.5, 0.1, 0.1});
    ...
    float*** A = grammar.get_A();
    float** B = grammar.get_B();

    float* A_flat = new float[N*N*N];
    float* B_flat = new float[N*M];

    float*** A_converted = new float**[N];
    float** B_converted = new float*[N];
    ...
    for(int i = 0; i < N; ++i){
        ...
        A_converted[ii] = new float*[N];
        for(int j = 0; j < N; ++j){
            jj = index_to_non_term[j];
            A_converted[ii][jj] = new float[N];
            for(int k = 0; k < N; ++k){
                kk = index_to_non_term[k];
                A_converted[ii][jj][kk] = A[i][j][k];
            }
        }
        B_converted[ii] = new float[M];
        for(int j = 0; j < M; ++j){
    ...
    Flat_in_out fao (A_flat, B_flat, N, M, terminals, 0);
