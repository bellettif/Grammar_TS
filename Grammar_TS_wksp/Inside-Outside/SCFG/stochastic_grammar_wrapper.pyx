...
def iterate_estimation_perturbated(np.ndarray A_proposal,
	...
	for iter in xrange(n_iterations):
		...
		c_A_proposal = np.maximum(c_A_proposal, np.power(1.0 / float(n_iterations + 1.0), dampen) * epsilon_A * np.ones((N, N, N), dtype = DTYPE))
		c_B_proposal = np.maximum(c_B_proposal, np.power(1.0 / float(n_iterations + 1.0), dampen) * epsilon_B * np.ones((N, M), dtype = DTYPE))
		for i in xrange(N):
