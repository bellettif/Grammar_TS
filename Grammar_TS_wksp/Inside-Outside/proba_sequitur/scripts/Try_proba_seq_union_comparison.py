...
ps = Proba_sequitur(build_samples = both_data_sets,
                    ...
                    max_rules = max_rules)
ps.infer_grammar()
ps.plot_stats('achu_oldo_not_filtered_%d_%s_%d' % (degree, keep_data, max_rules))
...
ps = Proba_sequitur(build_samples = selected_oldo_data_set,
                    ...
                    max_rules = max_rules)
ps.infer_grammar()
ps.plot_stats('oldo_not_filtered_%d_%s_%d' % (degree, keep_data, max_rules))
...
ps = Proba_sequitur(build_samples = selected_achu_data_set,
                    ...
                    max_rules = max_rules)
ps.infer_grammar()
ps.plot_stats('achu_not_filtered_%d_%s_%d' % (degree, keep_data, max_rules))
...
plt.savefig('Freqs_merged_not_filtered_%d_%s_%d.png' % (degree, keep_data, max_rules), dpi = 600)
plt.close()
