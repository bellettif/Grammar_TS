...
folder_path = 'Stochastic_results_p_01/'
        ...
        for filter_option, selected_achu_data_set, selected_oldo_data_set in filter_option_set:
            for T in T_set:
                for i_trial in range(10):
                    print "Doing degree = %d, max_rules = %d, filter_option = %s, T = %f, trial = %d" % \
                            (degree, max_rules, filter_option, T, i_trial)
                    keep_data = 'keep_data'
                    ...
                    ps.infer_grammar()
                    ps.plot_stats('achu_oldo_%s_%d_%s_%d_%f_%d' % 
                                  (filter_option, degree, keep_data, max_rules, T, i_trial),
                                  folder_path)
                    ...
                    ps = Proba_sequitur(build_samples = selected_oldo_data_set,
                                        ...
                                        p_deletion = p_deletion)
                    ps.infer_grammar()
                    ps.plot_stats('oldo_%s_%d_%s_%d_%f_%d' 
                                  % (filter_option, degree, keep_data, max_rules, T, i_trial),
                                  folder_path)
                    ...
                    ps = Proba_sequitur(build_samples = selected_achu_data_set,
                                        ...
                                        p_deletion = p_deletion)
                    ps.infer_grammar()
                    ps.plot_stats('achu_%s_%d_%s_%d_%f_%d' % 
                                  (filter_option, degree, keep_data, max_rules, T, i_trial),
                                  folder_path)
                    ...
                    plt.savefig(folder_path + ('Freqs_merged_%s_%d_%s_%d_%f_%d.png' % 
                                (filter_option, degree, keep_data, max_rules, T, i_trial)), dpi = 600)
                    plt.close()
                    print 'Done\n'
