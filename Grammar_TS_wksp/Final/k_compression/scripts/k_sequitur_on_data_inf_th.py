...
for k in [2, 4, 6, 8, 10, 12]:
    ...
    plt.savefig('Repetitions_entropy_seq_%d.png' % k, dpi = 600)
    plt.close()
    ...
    plt.ylabel('Post compression entropy')
    plt.xlabel('Pre compression entropy')
    plt.title('Sequitur (k = %d) (no repetitions of a) (size prop. to number of rules)' % k)
    plt.savefig('No_repetitions_entropy_seq_%d.png' % k, dpi = 600)
    plt.close()
